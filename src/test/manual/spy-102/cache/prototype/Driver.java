...
public class Driver implements Runnable {
	...
	private ConcurrentHashMap<String, Integer> keyMap = new
	...
	public Driver(StatCollector collector, int msgSize, int objectTTL,
		...
		//Build a character array of desired size
        	...
        	this.connector = new
                    CouchCacheConnector(host, collector, bucket);
    ...
    public void printStats(long intervalReportTime){
    	...
    	logger.info(
    			" putCnt= " + putCount +
    			" getCnt= " + getCount +
    			" putRate= " + dFormat.format(((double)
                            putCount)/intervalReportTime) +
    			" getRate=" + dFormat.format(((double)
                            getCount)/intervalReportTime) +
    			" putLatency= "
                            + ((putCount!=0) ? 
	...
	public void testFailover() {
        	int sleep =
'                   Integer.parseInt(System.getProperty("sleep", "100"));
        	int factor =
	...
	public static void main(String[] args) {
        ...
        try {
            ...
            driver = new
                 Driver(collector, msgSize, objectTTL,threadCount, keyPrefix, bucket);
