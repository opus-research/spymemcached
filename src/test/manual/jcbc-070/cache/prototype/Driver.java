...
public class Driver implements Runnable {
	...
	public Driver(StatCollector collector, int msgSize, int objectTTL, int threads, String keyPrefix, String bucket) throws Exception {
	 ...
	 */
	public void start(){
        	tpe = new ThreadPoolExecutor(threads,threads, 60, TimeUnit.SECONDS, new SynchronousQueue<Runnable>());
    ...
    public void storeObjects(StatCollector collector, String keySuffix) {
		...
		try{
	        ...
	        int noOfObjects = Integer.valueOf(p.getProperty("preload.noOfObjects","1000"));
	        int objectTTL   = Integer.valueOf(p.getProperty("preload.objectTTL", "300"));
    ...
    public void printStats(long intervalReportTime){
    	...
    	logger.info(
    			...
    			" putLatency=" + ((putCount!=0) ? collector.getPutLatency().get()/(putCount * 1000) : 0)+
    			" getLatency=" + ((getCount!=0) ? collector.getGetLatency().get()/(getCount * 1000) : 0)+
    			" PutErrCnt=" + collector.getPutErrorCount() +
    			" GetErrCnt=" + collector.getGetErrorCount() +
    			" GetStaleCnt=" + collector.getStaleCount() +
    			" UnAvail " + connector.getClient().getUnavailableServers()
    			);
    	collector.reset();
    }
	...
	public void testFailover() {
        	...
        	int factor = Integer.parseInt(System.getProperty("factor", "1"));
        	long counter = 0;
		try{
	...
	public void testStale() {
    	int sleep = Integer.parseInt(System.getProperty("sleep", "10"));
    	int factor = Integer.parseInt(System.getProperty("factor", "1000"));
    	long counter = 0;
		try{
			...
			for(int i=0; i< 100000; i++){
				...
				while(en.hasNext()) {
					try{
						String key = en.next();
						Integer ver = keyMap.get(key);
						POJO value = (POJO) couch.get(key);
						if(value != null && value.getVersion() != ver){
							//System.out.println("Stale " + key);
							collector.incrementStaleCount();
						}
						
						if(value == null) {
							//System.out.println(key + " ** " + ver);
							collector.incrementGetErrorCount();
							value = new POJO(ver, data); 
						}
	...
	public void testStoreWithoutDelete() {
        int sleep = Integer.parseInt(System.getProperty("sleep", "100"));
        int factor = Integer.parseInt(System.getProperty("factor", "1"));
        long counter = 0;
		try{
			...
			while(!finished) {
				try{
					String key = keyPrefix + System.nanoTime() + Thread.currentThread().getId();
					//Put the object
					couch.put(key, objectTTL, data);
					//Thread.sleep(50);
					Object obj = couch.get(key);
	...
	*/
	...
	public static void main(String[] args) {
        ...
        int numIntervals = Integer.parseInt(System.getProperty("intervals", "32000"));
        int intervalReportTime = Integer.parseInt(System.getProperty("itime", "5"));
        ...
        String keyPrefix = System.getProperty("keyprefix", "_1994190_RESG-plabq13.dev.sabre.com.1333635157880-9");
