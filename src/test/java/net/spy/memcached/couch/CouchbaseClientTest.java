...
public class CouchbaseClientTest extends TestCase {
	...
	private static final String SERVER_URI = "http://" + TestConfig.IPV4_ADDR + "/pools";
	...
	private static String generateDoc(String type, String small,
			String large) {
		return "{\"type\":\"" + type + "\"" +
		"\"small range\":\"" + small + "\"" +
		"\"large range\":\"" + large + "\"}";
	...
	public void testViewNoDocs() throws Exception {
		Query query = new Query();
		View view = client.getView(DESIGN_DOC_W_REDUCE, VIEW_NAME_W_REDUCE);
		HttpFuture<ViewResponseNoDocs> future = client.queryAndExcludeDocs(view, query);
		...
		assert response.size() == items.size() : "Failed: " + future.getStatus().getMessage();
	}
	
	public void testReduce() throws Exception {
		Query query = new Query();
		View view = client.getView(DESIGN_DOC_W_REDUCE, VIEW_NAME_W_REDUCE);
		HttpFuture<ViewResponseReduced> future = client.queryAndReduce(view, query);
		...
		while (itr.hasNext()) {
			...
			assert Integer.valueOf(row.getValue()) == items.size() :
				"Failed: " + future.getStatus().getMessage();
		}
	}
	...
	class TestingClient extends CouchbaseClient {
		...
		public HttpFuture<String> asyncHttpPut(String uri, String document)
			...
			HttpOperationImpl op = new PutOperationImpl(request, new PutCallback() {
				String json;
				@Override
	...
	class PutOperationImpl extends HttpOperationImpl
