	...
	protected String getEntityString(HttpResponse response) {
		if (!isTimedOut() && !hasErrored() && !isCancelled()) {
			try {
				...
				exception = new OperationException(OperationErrorType.GENERAL, "Bad http headers");
				errored = true;
			} catch (IOException e) {
				exception = new OperationException(OperationErrorType.GENERAL, "Error reading response");
				errored = true;
			} catch (IllegalArgumentException e) {
				exception = new OperationException(OperationErrorType.GENERAL, "No entity");
				errored = true;
			}
		}
		return null;
	...
	protected OperationStatus parseViewForStatus(String json)
		...
		try {
			...
			if (base.has("error")) {
				...
				if (base.has("reason")) {
					error += " - " + base.getString("reason");
				}
				return new OperationStatus(false, error);
			} else {
				return new OperationStatus(true, "OK");
			}
