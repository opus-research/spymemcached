...
public class CouchbaseClient extends MembaseClient {
	...
	public List<View> getViews(String designDocName)
			throws InterruptedException, ExecutionException {
		String uri = "/" + bucketName + "/_design/" + designDocName;
		String designDocJson = asyncHttpGet(uri).get();
		List<View> views;
		try {
			views = DocParserUtils.parseDesignDocumentForViews(bucketName, designDocName, designDocJson);
