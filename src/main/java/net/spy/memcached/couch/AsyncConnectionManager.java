...
public class AsyncConnectionManager extends SpyObject {
    ...
    static class ManagedClientHandler implements NHttpClientHandler {
        ...
        public void inputReady(NHttpClientConnection conn, ContentDecoder decoder) {
        	System.out.println("Decoding done: " + decoder.isCompleted() + " " + (decoder instanceof ChunkDecoder));
        	ByteBuffer buf = ByteBuffer.allocate(1024);
        	if (conn.getHttpResponse().getEntity().isChunked()) {
        	try {
        	System.out.println(decoder.read(buf));
			} catch (IOException e) {
    ...
    static class ConnRequestCallback extends SpyObject
        ...
        public void completed(SessionRequest request) {
            getLogger().info(request.getRemoteAddress() +  " - Session request successful");
        }

        public void cancelled(SessionRequest request) {
        	getLogger().info(request.getRemoteAddress() +  " - Session request cancelled");
            this.request.cancel();
        }

        public void failed(SessionRequest request) {
        	getLogger().error(request.getRemoteAddress() +  " - Session request failed");
        ...
        public void timeout(SessionRequest request) {
            getLogger().info(request.getRemoteAddress() +  " - Session request timed out");
            this.request.cancel();
        }
    }
